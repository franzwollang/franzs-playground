...
# For any given query label, the memory complexity of this implementation is o(1). The time complexity strictly upper-bounded by logN since the maximum number of steps is a single path from the root to a leaf node which for a balanced binary tree is strictly of length logN; the average time complexity is rather lower since asymptotically half of all nodes in the tree are not leaf nodes, and thus the number of steps is smaller, plus there are optimizations like checking the right boundary range.
