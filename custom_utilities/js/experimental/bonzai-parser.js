...
function satisfy(test){
  return (input) => {
    return input.length === 0 ? fail(input): test(input[0]) ? success(input[0], input.slice(1)): fail(input);
  }
}
...
function then(first, second){
  return (input) => {
    return flatMap(
      first(input),
      ([r, remainder]) => second(remainder).map(
  ...
  return input =>
    alt(
      ...
      map(
        ...
        then(
          left,
          then(bracket(left, middle, right), right)
        )
      )
    )(input);
}
