...
//This is better than the o(n) time complexity and o(n*m) space complexity of the straightforward approach of memoizing all intermediate values --potentially a quadratic reduction in space usage for n same order of magnitude as m. It's also obviously much better than the o(2^m) time and space complexity of the naive branching recursive approach.

//Creating a fully recursive version of this algorithm would be impossible with existing languages, given the loopy structure of the term dependencies. The language used would have to support a generalized directed graph function call structure with frame merging and splitting (rather than a standard stack function call tree structure with only splitting).

//Along similar lines, a purely functional version would be difficult without reverting back to o(n*m) space complexity by iterating over an array of "loops" number of arrays of max size "maxTerms". Would have to be able to paramterize the number of loops as a single number rather than as an outer array of arrays. Could use iterators that return new iterators? Same problem; would need a loop in one form or another to iterate through them, or could use recursion in a language with tail recursion (but that again just converts it into a loop).
