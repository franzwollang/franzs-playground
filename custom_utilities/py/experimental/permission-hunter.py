...
def set_containment(set_collection, guarantee_sort=False):
    if isinstance(set_collection, dict):
        ...
        raise ValueError(
            "Argument must be either a dictionary of sets or a list of pairs of strings and sets."
        )
    ...
    for role, list in tree.items():
        ...
        for element in list:
            if isinstance(element, dict):
                lower_cascade = lower_cascade.union(
                    cascade_permissions(inv_dict, element, role_sets, root=False)
                )
    ...
    def convert(self, access_grove = None):
        def hierarchy(ptree, node_dict, parent = None, isroot = False):
            if not ptree.children:
                ...
                node_dict[ptree.label] = role_tree
                role_tree.children = [
                    hierarchy(child, node_dict, parent = role_tree)
